package com.cts.fasttack.crypto.server.util.certverification;
...
public class CertificateVerifier {
     ...
     *            anchors" and intermediate CA certificates that will be used as
     *            part of the certification chain. All self-signed certificates are
     *            considered to be trusted root CA certificates. All the rest are
     *            considered to be intermediate CA certificates.
     * @param isCRLVerificationMandated
     *            - if false then no exception is thrown if it is impossible to verify CRL
     * @return the certification chain (if verification is successful)
     * @throws CertificateVerificationException
     *             - if the certification is not successful (e.g. certification path
     *             cannot be built or some certificate in the chain is expired or
     *             CRL checks are failed)
     */
    public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert,
            Set<X509Certificate> additionalCerts, boolean isCRLVerificationMandated) throws CertificateVerificationException {
        try {
            ...
            CRLVerifier.verifyCertificateCRLs(cert, isCRLVerificationMandated);
     ...
     *             - if the verification is not successful (e.g. certification path
    ...
    private static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert,
        ...
        CertStore intermediateCertStore = CertStore.getInstance("Collection",
                new CollectionCertStoreParameters(intermediateCerts), bcp);
        pkixParams.addCertStore(intermediateCertStore);
        
        pkixParams.addCertPathChecker(new CriticalExtensionsRemovingCertPathChecker());

        // Build and verify the certification chain
        CertPathBuilder builder = CertPathBuilder.getInstance("PKIX", bcp);
        PKIXCertPathBuilderResult result = (PKIXCertPathBuilderResult) builder.build(pkixParams);
        return result;
    }

    private static class CriticalExtensionsRemovingCertPathChecker extends PKIXCertPathChecker {
