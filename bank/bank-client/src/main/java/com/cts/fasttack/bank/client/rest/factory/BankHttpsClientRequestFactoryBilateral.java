 ...
 * @author Anton Leliuk
 */
@Component("bankHttpsClientRequestFactoryBilateral")
@ConfigurationProperties("spring.client")
@ConditionalOnProperty(prefix = "spring.client", value = "mutualTls")
public class BankHttpsClientRequestFactoryBilateral extends SimpleClientHttpRequestFactory implements InitializingBean {
    ...
    private KeyStore.Builder createJksKeyStoreBuilder(Resource keyStore, String password) throws IOException {
        return KeyStore.Builder.newInstance("JKS", null, keyStore.getFile(), new KeyStore.CallbackHandlerProtection(new PasswordCallbackHandler(password)));
    }
