package com.cts.fasttack.bank.client.ws.interceptor;
...
/**
 * Interceptor that log WS request and response messages
 * @author Anton Leliuk
 */
    ...
    public boolean handleRequest(MessageContext messageContext) throws WebServiceClientException {
        if(log.isDebugEnabled() || log.isTraceEnabled()){
            ...
            logPayload(message, new LoggingMessage(LoggingMessage.REQUEST_MESSAGE, id)
                    .remoteAddress(uri)
                    .header(StreamSupport.stream(headersIterable.spliterator(), false).map(mimeHeader -> mimeHeader.getName() + " = " + mimeHeader.getValue()).collect(Collectors.joining(",")))
    ...
    private void afterCompletion(MessageContext messageContext){
        if(log.isDebugEnabled() || log.isTraceEnabled()){
            ...
            if(StringUtils.isNotEmpty(id)){
                ...
                Iterable<MimeHeader> headersIterable = () -> headers;
                logPayload(message,
                        new LoggingMessage(LoggingMessage.RESPONSE_MESSAGE, id)
                                .message(message.getFaultReason())
                                .header(StreamSupport.stream(headersIterable.spliterator(), false).map(mimeHeader -> mimeHeader.getName() + " = " + mimeHeader.getValue()).collect(Collectors.joining(","))));
            }
        }
    }
